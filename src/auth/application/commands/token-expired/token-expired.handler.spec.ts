...
import { TokenExpiredCommand, TokenExpiredHandler } from 'auth/application/commands/token-expired';
...
describe('TokenExpiredHandler', () => {
  ...
  describe('execute', () => {
    it('should execute TokenExpiredCommand', async () => {
      ...
      expect(service.verify).toBeCalledWith(token, { expiresIn: '7d', secret: 'jwt-secret-refreshtoken' });
    });
  });
});
